...
from gbmi.exp_argmax_of_n import SEEDS, SELECTED_SEED
...
@dataclass
class ArgmaxOfN(ExperimentConfig):
    # Model config
    n_layers: int = 1
    n_heads: int = 1
    d_model: int = 32
    ...
    def get_eos_token(self) -> Optional[int]:
        return self.d_vocab if self.use_end_of_sequence else None

    def get_training_wrapper(self):
        return ArgmaxOfNTrainingWrapper

    def get_datamodule(self):
        return ArgmaxOfNDataModule

    def get_summary_slug(self, config: Config[ArgmaxOfN]) -> str:
        ...
        return (
            f"ArgmaxOf{config.experiment.seq_len}-{config.train_for[0]}-{config.train_for[1]}"
    ...
    def get_ground_truth(
        ...
        if self.use_end_of_sequence:
            x = x[..., :-1]
        if self.nth_argmax == 1:
            return x.max(dim=-1).indices
        else:
            return torch.topk(
                x, self.nth_argmax, dim=-1, sorted=True, largest=True
            ).indices[..., self.nth_argmax - 1]


ARGMAX_OF_2_CONFIG: Config[ArgmaxOfN] = Config(
    experiment=ArgmaxOfN(
...
ARGMAX_OF_10_SINGLE_CONFIG: Config[ArgmaxOfN] = Config(
    experiment=ArgmaxOfN(
...
def ARGMAX_OF_4_CONFIG(seed: int) -> Config[ArgmaxOfN]:
    return Config(
        experiment=ArgmaxOfN(
            # act_fn=None,
            attn_only=True,
...
def ARGMAX_OF_5_CONFIG(seed: int, *, deterministic: bool = False) -> Config[ArgmaxOfN]:
    return set_params(
...
def ARGMAX_OF_10_CONFIG(
    seed: int, d_vocab: int = 64, *, deterministic: bool = False
) -> Config[ArgmaxOfN]:
    return set_params(
        ARGMAX_OF_4_CONFIG(seed),
        {
...
class ArgmaxOfNTrainingWrapper(TrainingWrapper[ArgmaxOfN]):
    def __init__(self, config: Config[ArgmaxOfN], model: HookedTransformer):
    ...
    @staticmethod
    def build_model_config(config: Config[ArgmaxOfN]) -> HookedTransformerConfig:
        return HookedTransformerConfig(
            ...
            + (1 if config.experiment.use_end_of_sequence else 0),
            d_vocab_out=config.experiment.seq_len,
    ...
    @staticmethod
    def build_model(config: Config[ArgmaxOfN]) -> HookedTransformer:
        model = HookedTransformer(ArgmaxOfNTrainingWrapper.build_model_config(config))
    ...
    def acc_fn(
        ...
        return (
            ArgmaxOfNTrainingWrapper.acc_fn_per_seq(logits, labels)
            .float()
            .mean()
            .item()
        )
...
class ArgmaxOfNIterableDataset(
    IterableDataset[
...
class ArgmaxOfNDataModule(DataModule):
    data_train: Dataset[
    ...
    def __init__(self, config: Config[ArgmaxOfN]):
        super().__init__(config)
        self.config = config
        # self.model_config = ArgmaxOfNTrainingWrapper.build_model_config(config)
    ...
    def build_dataset(
        ...
        if isinstance(cfg, IterableDatasetCfg):
            base_dataset = ArgmaxOfNIterableDataset(
...
def config_of_argv(argv=sys.argv) -> tuple[Config[ArgmaxOfN], dict]:
    parser = argparse.ArgumentParser(
        description="Train a model with configurable attention rate."
    )
    add_force_argument(parser)
    add_no_save_argument(parser)
    # add --argmax-of N argument accepting 2 and 10
    parser.add_argument(
        "--argmax-of",
        metavar="N",
        type=int,
        default=10,
        help="The length of the list to take the argmaximum of.",
    )
    parser.add_argument(
        "--force-adjacent-gap",
        metavar="K",
        type=str,
        action="append",
        help="For --argmax-of 2, include all sequences (n, n±K) in training set. Accepts int and comma-separated-list.",
    )
    parser.add_argument(
        "--training-ratio",
        type=float,
        default=0.7,
        help="For --argmax-of 2, the fraction of sequences to include in training.",
    )
    parser.add_argument(
        "--use-log1p",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Pick the maximum value first, then fill in the rest of the sequence. Only meaningful for --argmax-of N > 2.",
    )
    parser.add_argument(
        "--use-kaiming-init",
        action=argparse.BooleanOptionalAction,
    ...
    config.experiment = ArgmaxOfNTrainingWrapper.update_config_from_model_config(
        config.experiment,
        update_HookedTransformerConfig_from_args(
            config,
            ArgmaxOfNTrainingWrapper.build_model_config(config),
            args,
...
# # %%
# runtime, model = main([i for i in "train  --argmax-of 2 --non-deterministic --train-for-epochs 3000 --validate-every-epochs 20 --force-adjacent-gap 0,1,2 --use-log1p --training-ratio 0.095 --weight-decay 1.0 --betas 0.9 0.98 --optimizer AdamW --use-end-of-sequence --force load".strip().split(" ") if i])
# # %%
