...
torchviz.make_dot(
    model(
        torch.cat(
            [
                torch.randint(0, model.cfg.d_vocab_out, (1, 2)),
                torch.full((1, 1), model.cfg.d_vocab - 1),
            ],
            dim=-1,
        )
    ),
...
xs = torch.cat(
    [
        torch.randint(0, model.cfg.d_vocab_out, (1000, 2)),
        torch.full((1000, 1), model.cfg.d_vocab - 1),
    ],
    dim=-1,
)
print(xs)
...
loss = (
    ys[:, -1, :]
    .log_softmax(dim=-1)[torch.arange(xs.shape[0]), xs[:, :-1].min(dim=-1).values]
    .mean()
)
# %%
px.line(
    (
        (model.W_E[-1] + model.W_pos[-1])
        @ model.W_Q[0, 0]
        @ model.W_K[0, 0].T
        @ (
            model.W_E[:-1]
            - model.W_E[-1]
            + model.W_pos[:-1].mean(dim=0)
            - model.W_pos[-1]
        ).T
    ).detach()
).show()
...
# %%
from gbmi.exp_max_of_n.train import MaxOfNTrainingWrapper

optim = MaxOfNTrainingWrapper(cfg, model).configure_optimizers()
px.line(
    (
        (model.W_E[-1] + model.W_pos[-1])
        @ model.W_Q[0, 0]
...
# %%
W_Egrad = model.W_E.grad
W_posgrad = model.W_pos.grad
W_Qgrad = model.blocks[0].attn.W_Q.grad
W_Kgrad = model.blocks[0].attn.W_K.grad
W_E2 = model.W_E + model.W_E.grad
W_pos2 = model.W_pos + model.W_pos.grad
W_Q2 = model.blocks[0].attn.W_Q + model.blocks[0].attn.W_Q.grad
W_K2 = model.blocks[0].attn.W_K + model.blocks[0].attn.W_K.grad
px.line(
    (
        (
            (W_E2[-1] + W_pos2[-1])
            @ W_Q2[0]
            @ W_K2[0].T
            @ (W_E2[:-1] - W_E2[-1] + W_pos2[:-1].mean(dim=0) - W_pos2[-1]).T
        )
        - (
            (model.W_E[-1] + model.W_pos[-1])
            @ model.W_Q[0, 0]
