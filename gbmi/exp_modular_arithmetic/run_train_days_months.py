...
with tqdm(SEEDS, desc="Seed", position=0, leave=False) as seed_pbar:
    with tqdm((7, 12), desc="p", position=1, leave=False) as p_pbar:
        with tqdm((PIZZA_CONFIG, CLOCK_CONFIG), position=2, leave=False) as cfg_pbar:
            with tqdm(
                (True, False), desc="use eos", position=3, leave=False
            ) as eos_pbar:
